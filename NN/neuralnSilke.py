...
results = numpy.array(SP["Generated"])
...
train_X = matrix[:train_size,:].transpose()
test_X =  matrix[train_size:,:].transpose()
...
pred = tf.Variable(tf.random_normal([1]), name='bias')
for pow_i in range(1, 5):
    W = tf.Variable(tf.random_normal([1]), name='weight_%d' % pow_i)
    pred = tf.add(tf.multiply(tf.pow(X, pow_i), W), pred)
...
with tf.Session() as sess:
    ...
    # Graphic display
    plt.plot(range(test_Y.shape[0]),test_Y,label="Original Data")
    plt.plot(range(test_Y.shape[0]),pred,label="Predicted Data")
    plt.legend(loc='best')
